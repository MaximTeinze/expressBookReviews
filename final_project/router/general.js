...
public_users.post("/register", (req,res) => {
    ...
    return res.status(404).json({message: "Registration failied: Invalid username and/or password."});
});
...
public_users.get('/isbn/:isbn',function (req, res) {
    ...
    isbnPromise.then(() => {
        ...
        return res.status(300).json({message: "Book does not exist"});
    })
...
public_users.get('/author/:author',function (req, res) 
{
    ...
    authorPromise.then(() => {
        ...
        if(!booksByAuthor.length) return res.status(300).json({message:"Book not available"});
...
public_users.get('/title/:title',function (req, res) {
    ...
    titlePromise.then(() => {
        ...
        if(!booksByTitle.length) return res.status(300).json({message:"Book not available"});
